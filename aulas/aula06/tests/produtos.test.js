...
describe('Teste da API Produtos', () => {
  ...
  test("POST / deve retornar 422 um array JSON", async () => {
  ...
  test("GET /id deve retornar 200 um objeto JSON", async () => {
  ...
  test("GET /id deve retornar 404 um objeto JSON", async () => {
  ...
  test("PUT /id deve retornar 200 um objeto JSON", async () => {
  ...
  test("PUT /id deve retornar 404 um objeto JSON", async () => {
    const response = await request.put("/produtos/100");
    expect(response.status).toBe(404);
    expect(response.type).toBe("application/json");
    expect(response.body).toHaveProperty("msg", "Produto não encontrado");
  });
  test("DELETE /id deve retornar 204 e sem corpo", async () => {
    const response = await request.delete("/produtos/1");
    expect(response.status).toBe(204);
    expect(response.type).toBe("");
    expect(response.body).toEqual({});    
  });
  test("DELETE /id deve retornar 404 um objeto JSON", async () => {
